...
public:
    ...
    std::vector<std::vector<int>> zigzagLevelOrder(TreeNode* root) {
        ...
        while (!nodes_to_visit.empty()) {
            auto [node, level] = nodes_to_visit.front();
            nodes_to_visit.pop();

            if (level >= level_sorted_values.size()) {
                level_sorted_values.emplace_back();
            }
            level_sorted_values[level].emplace_back(node->val);

            if (node->left) {
                nodes_to_visit.emplace(node->left, level + 1);
            }
            if (node->right) {
                nodes_to_visit.emplace(node->right, level + 1);
            }
        }
