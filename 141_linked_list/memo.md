...
- 関数の最後に `return false` を書かなかった場合、x64 系の CPU で動かした場合は、関数を抜けたタイミグで rax レジスターに置かれていた値が戻り値として返る。コンパイラオプションに `-Werror=return-type` を付けるとエラーが出るようになるとのこと。

    - rax レジスタは x64 環境において関数の引数や戻り値を保持する役割を持っている。

    - この周辺の具体的な知識については本を読んで勉強する。

- `->` を構造体やクラスのポインタが指し示すメンバ（変数や関数）にアクセスするために用いる場合は、両側にスペースを空けないことが多いとのこと。他方、戻り値の型を後置する際は両側にスペースを空けることが多い。

    - [C++のレファレンス](https://cpprefjp.github.io/lang/cpp11/trailing_return_types.html) を読もう。
